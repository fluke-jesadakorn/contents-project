    ...
    <div>
      {title !== null && (
        <div className="flex items-baseline justify-between gap-2">
          <div className={`text-xs uppercase tracking-widest ${headerTone}`}>
            {title}
          </div>
        ...
        <div className="min-w-0 flex-1 text-xs leading-tight text-mute">
          <p>
            ...
            {second && (
              <>
                {', '}
                <span className="font-semibold text-ink">{second.fullname}</span>
              </>
            )}
            {approvers.length > 2 && (
              <>
          ...
          <p className="mt-0.5 text-xs uppercase tracking-widest text-[var(--text-faint)]">
            {first
              ? multiRole
      ...
      {actedUsers.length > 0 && (
        <div className="mt-3 text-xs uppercase tracking-widest text-positive/80">
