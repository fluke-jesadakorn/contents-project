...
import { Badge, Icon, Panel, Tabs } from '@/components/ui';
  ...
  return (
    <Panel padding="none" className="folio-html-report my-2 overflow-hidden border-accent/40">
      <header className="flex flex-wrap items-center gap-2 border-b border-rule bg-paper-2 px-3 py-2">
        ...
        <Badge tone="positive"><Icon name="check" size={12} />source: ask_sql</Badge>
        ...
        <div className="ml-auto flex flex-wrap gap-1">
          {(['html', 'css', 'js', 'full'] as const).map((key) => <button key={key} type="button" onClick={() => copy(key)} className="inline-flex h-7 items-center gap-1 rounded-md border border-rule bg-paper px-2 font-mono text-xs text-ink-2 hover:bg-paper-3"><Icon name={copied === key ? 'check' : 'copy'} size={12} />{key}</button>)}
        </div>
      </header>
