...
import { Badge, Panel, Status, type BadgeTone, type StatusTone } from '@/components/ui';
...
const META: Record<ToolCardProps['status'], { icon: LucideIcon; tone: StatusTone; badge: BadgeTone; label: string }> = {
  running: { icon: LoaderCircle, tone: 'caution', badge: 'caution', label: 'running' },
  ok: { icon: CheckCircle, tone: 'positive', badge: 'positive', label: 'ok' },
  empty: { icon: Minus, tone: 'neutral', badge: 'neutral', label: 'empty' },
  error: { icon: CircleAlert, tone: 'critical', badge: 'critical', label: 'error' },
};
  ...
  return (
    <Panel padding="none" className="my-1 overflow-hidden">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs hover:bg-paper-3">
        <IconCmp size={15} className={props.status === 'running' ? 'animate-spin text-caution' : ''} />
        ...
        <ChevronRight size={14} className={['ml-auto transition-transform', open ? 'rotate-90' : ''].join(' ')} />
      </button>
      {open && details && (
        <div className="space-y-2 border-t border-rule bg-paper px-3 py-2 font-mono text-xs text-ink-2">
          ...
          {props.error && <Status tone="critical"><CircleAlert size={14} />{props.error}</Status>}
