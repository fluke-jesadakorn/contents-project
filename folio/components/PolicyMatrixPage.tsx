...
const LOCALE_FOR_LABELS = { th: 'th', de: 'de' } as const;

export function PolicyMatrixPage({ stages, personas, canEdit, actorName, policies }: Props) {
  const router = useRouter();
  const t = useTranslations();
  const personLocale = (typeof window !== 'undefined' && window.localStorage.getItem('folio.lang')) || 'th';
  const locale: 'th' | 'de' = personLocale === 'de' ? 'de' : 'th';
    ...
    <div className="space-y-5 animate-fade-in">
      <header className="flex items-end justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          <h2 className="text-base font-black uppercase tracking-widest text-slate-100 flex items-center gap-2">
            ...
            <T id="policy.matrix.title" hideSecondary /> · <T id="policy.matrix.grantsN" hideSecondary values={{ n: totalGrants }} />
          </h2>
          <p className="mt-1 text-xs text-slate-400 leading-relaxed">
            <T id="policy.matrix.subtitle" hideSecondary />
          </p>
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            <span className="text-xs font-mono uppercase tracking-widest text-slate-500">
              <T id="policy.matrix.chainOrder" hideSecondary />
            </span>
            {stages.map((s, i) => (
              <React.Fragment key={s.perm}>
                <span className="text-xs font-mono text-indigo-300">
                  {STAGE_LABELS[s.stage]?.icon ?? '·'} <T id={`waybill.stage.${s.stage}`} hideSecondary />
                </span>
        ...
        <div className="text-xs font-mono text-slate-500">
          {canEdit ? <T id="policy.matrix.editGranted" hideSecondary /> : <T id="policy.matrix.viewOnly" hideSecondary />} · {actorName || '—'}
        </div>
      </header>
              ...
              <tr>
                <th className="text-left px-3 py-2.5 sticky left-0 bg-slate-900/95 backdrop-blur z-10 min-w-[200px]">
                  <span className="text-xs font-black uppercase tracking-widest text-slate-300">
                    <T id="policy.matrix.colPersona" hideSecondary />
                  </span>
                </th>
                {stages.map((s) => (
                  <th key={s.perm} className="px-2 py-2.5 min-w-[110px] text-center">
                    <div className="text-xs font-mono text-slate-200 font-bold flex flex-col items-center gap-0.5">
                      ...
                      <span className="leading-tight">
                        <T id={`waybill.stage.${s.stage}`} hideSecondary />
                      </span>
                    </div>
                  </th>
                ))}
                <th className="px-3 py-2.5 min-w-[120px] text-right">
                  <span className="text-xs font-mono uppercase tracking-widest text-slate-400">
                    <T id="policy.matrix.colUsers" hideSecondary />
                  </span>
                </th>
                {canEdit && (
                  <th className="px-3 py-2.5 min-w-[160px] text-right">
                    <span className="text-xs font-mono uppercase tracking-widest text-slate-400">
                      <T id="policy.matrix.colActions" hideSecondary />
                    </span>
                  </th>
                )}
              </tr>
                      ...
                      return (
                        <td key={s.perm} className="px-2 py-2 text-center">
                          <button
                            ...
                            aria-pressed={on}
                            aria-label={`${personaLabel(p, locale)} ${on ? t('policy.matrix.has') : t('policy.matrix.doesNotHave')} ${s.stage.replace(/_/g, ' ')}`}
                            className={[
                    ...
                    {canEdit && (
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex items-center gap-1.5">
                          ...
                          >
                            <T id="policy.matrix.reset" hideSecondary />
                          </button>
                          <button
                            type="button"
                            disabled={!isDirty || saving}
                          ...
                          >
                            {saving ? <T id="policy.matrix.saving" hideSecondary /> : <T id="policy.matrix.save" hideSecondary />}
                          </button>
                        </div>
                      </td>
                    )}
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4 text-sm text-slate-400 leading-relaxed">
        <span className="text-xs uppercase tracking-widest font-bold text-slate-300 mr-2">
          <T id="policy.matrix.howItWorks" hideSecondary />
        </span>
        <T id="policy.matrix.howItWorksBody" hideSecondary />
      </div>
