  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Pick language"
      ...
      <div className="glass-panel rounded-sm p-5 w-[calc(100vw-2rem)] max-w-sm">
        <h2 className="font-display text-lg text-ink"><T id="chrome.langPickerTitle" hideSecondary /></h2>
        <p className="text-xs text-mute font-sans mb-4 mt-1"><T id="chrome.langPickerHint" hideSecondary /></p>
        <div className="grid grid-cols-2 gap-2">
          <button type="button" onClick={() => onPick('th')}
            ...
            <span className="text-sm font-medium"><T id="chrome.thai" hideSecondary /></span>
          </button>
          <button type="button" onClick={() => onPick('de')}
            className="glass-panel flex flex-col items-center gap-1 rounded-sm p-4 hover:border-accent text-ink">
            <span className="font-display text-2xl">DE</span>
            <span className="text-sm font-medium"><T id="chrome.german" hideSecondary /></span>
          </button>
        </div>
      </div>
    </div>
