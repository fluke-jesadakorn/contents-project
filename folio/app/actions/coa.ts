...
function canActAtAccountingReview(
  ...
  if (matchPerm(actor.permissions, `stage:${current_stage}:act::allow`)) return true;
  if (matchPerm(actor.permissions, `stage:${current_stage}:act:all::allow`)) return true;
...
export async function applyCoaSuggestionAction(
  raw: ApplyCoaSuggestionArgs,
): Promise<ApplyCoaResult> {
  const parsed = CoaApplyForm.safeParse(raw);
  if (!parsed.success) {
  ...
  if (acct.normal_side !== args.normalSide) {
  ...
  const upd = await query(
    ...
    [args.code, args.itemId, args.expenseId],
  );
  if (upd.rowCount === 0) {
