...
import type { InboxScope } from '@folio-lib/waybill/queries';
...
export function InboxFilters({ current, counts, lang: _lang = 'en' }: Props) {
  const specs: Array<Omit<PillSpec, 'count'>> = [
    { scope: 'waiting',  href: '/inbox?scope=waiting',  icon: '⚡', label: 'Waiting on me' },
    { scope: 'watching', href: '/inbox?scope=watching', icon: '🔔', label: 'Watching' },
    { scope: 'all',      href: '/inbox?scope=all',      icon: '🗂',  label: 'All' },
  ];
