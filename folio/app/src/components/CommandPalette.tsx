...
import { type RoleName } from '@folio-lib/org/display';
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  // Suppress unused-var lint for `openTiles` / `lockedTiles` placeholders that
  // remained from the legacy tab/action gating path; they're set in the
  // destructure above and intentionally unused now that the catalog is static.
  void openTiles;
  void lockedTiles;
          ...
          {finalList.length === 0 ? (
            <div className="px-3 py-12 text-center">
              ...
              <div className="text-xs text-slate-400">
                {aiBusy ? '✨ Asking AI…' : `No commands found for "${query}"`}
              </div>
            </div>
          ) : (
