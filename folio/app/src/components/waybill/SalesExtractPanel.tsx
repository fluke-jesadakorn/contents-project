...
export function SalesExtractPanel({ lang, onUse }: Props) {
  ...
  return (
    <section className="mt-4 rounded-2xl border border-fuchsia-500/30 bg-fuchsia-950/15 p-4">
      <h3 className="mb-1 text-xs font-mono uppercase tracking-widest text-fuchsia-300">Paste email / chat → draft SO</h3>
      <p className="mb-2 text-xs text-slate-500">AI extracts customer + line items from free text.</p>
      <textarea
        value={text}
        onChange={(e) => setText(e.target.value)}
        rows={3}
        placeholder='e.g. Customer ABC wants 50 units of Product X at 100 THB each, Net 30.'
      ...
      {draft && (
        <div className="mt-3 space-y-2">
          <div className="text-xs text-slate-300">
            <span className="font-mono text-slate-500">customer:</span> {draft.customer_name}
            {draft.customer_code && <span className="ml-2 font-mono text-cyan-300">({draft.customer_code})</span>}
            <span className="ml-2 font-mono text-slate-500">terms:</span> {draft.payment_terms}
            <span className="ml-2 font-mono text-slate-500">conf:</span> {Math.round((draft.confidence ?? 0) * 100)}%
          </div>
