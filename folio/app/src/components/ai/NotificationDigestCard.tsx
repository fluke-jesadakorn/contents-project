import 'server-only';
import { generateDigest, loadLatestDigest, type NotificationDigest } from '@folio-lib/ai/digest';
  ...
  return (
    <div className="hidden lg:flex flex-col gap-1 rounded-lg border border-glass-border bg-surface-glass-heavy px-3 py-1.5 max-w-[300px]">
      <div className="flex items-center gap-2">
        <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400">Today&apos;s digest</span>
        <span className={`rounded-full border px-1.5 py-0.5 text-[10px] font-mono uppercase tracking-wide ${sevCls}`}>{digest.severity}</span>
        <span className="ml-auto text-[10px] font-mono text-slate-500">· {digest.sourceCount} events</span>
      </div>
      {bullets.length > 0 ? (
