...
export function PolicyLintButton({ policies }: Props) {
  ...
  return (
    <section className="mt-4 rounded-2xl border border-amber-500/30 bg-amber-950/15 p-4">
      <h3 className="mb-1 text-xs font-mono uppercase tracking-widest text-amber-300">AI Policy Lint</h3>
      <p className="mb-3 text-xs text-slate-500">Check a policy for internal contradictions, unreachable stages, and chain gaps.</p>
      ...
      {findings.length > 0 && (
        <div className="mt-3 space-y-2">
          <div className="text-xs text-slate-400">Findings for <span className="font-mono text-cyan-300">{policyName}</span>:</div>
      ...
      {!error && findings.length === 0 && policyName && (
        <div className="mt-3 rounded border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-200">No issues found.</div>
      )}
    </section>
  );
}