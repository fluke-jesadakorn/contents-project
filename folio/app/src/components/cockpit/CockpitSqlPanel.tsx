...
export function CockpitSqlPanel() {
  ...
  return (
    <section className="rounded-2xl border border-cyan-500/30 bg-cyan-950/20 p-5">
      <h3 className="mb-2 text-xs font-mono uppercase tracking-widest text-cyan-300">Ask the books</h3>
      <p className="mb-3 text-xs text-slate-500">Read-only SQL against an allow-listed set of finance, sales, and HR tables.</p>
      <textarea
        value={q}
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {result.rows.length === 0 ? (
                  <tr><td colSpan={result.columns.length} className="px-2 py-2 text-center text-slate-500">No rows</td></tr>
