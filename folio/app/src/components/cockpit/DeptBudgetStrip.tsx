...
import departmentsDict from '@folio-lib/i18n/departments';
...
function localizeDeptName(deptId: string, fallbackEn: string, locale: 'th' | 'de'): BilingualText {
  const family = deptId.split('-').slice(0, 2).join('-');
  const dictKey = `dept-${family.replace('dept-', '')}`;
  return getTextServer(departmentsDict, dictKey, locale).en
    ? getTextServer(departmentsDict, dictKey, locale)
    : { en: fallbackEn };
}
  ...
  return (
    <div className="space-y-2.5">
      {rows.map((row) => {
        ...
        const label = localizeDeptName(row.dept_id, row.dept_name, locale);
        return (
