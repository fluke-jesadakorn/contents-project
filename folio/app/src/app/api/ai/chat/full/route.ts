...
import { DEFAULT_CHAT_MODEL, DEFAULT_THINKING, THINKING_PRESETS } from '@folio-lib/ai/defaults';
import { parseChartBlocks } from '@/components/chat/chartContract';
import { parseHtmlBlocks } from '@folio-lib/ai/htmlContract';
import { parseSqlBlocks } from '@folio-lib/ai/sqlContract';
import { askSql } from '@folio-lib/ai/sql';
import { appendMessage, createSession } from '@/lib/chat/history';

export const dynamic = 'force-dynamic';
export const runtime = 'nodejs';

const SYS = `You are Folio AI — a finance assistant for a Thai ERP.
...
SQL — when the user asks about folio data (expenses, vendors, waybills, customers, GL), append a single read-only query request:
...
export async function POST(req: NextRequest) {
  ...
  const { messages, sessionId, model, thinking, lang } = body as {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        for await (const chunk of invokeStream('chat:full', 'chat', {
          messages,
        ...
        send('meta', { sessionId: sid, modelName, latencyMs, blocks });

        await appendMessage(actor.id, sid, {
          role: 'assistant', content: full, blocks, modelName, latencyMs,
        });
        controller.close();
      } catch (e: any) {
