...
import { tileFromRow, GROUP_LABEL, groupLabelBi, type TileRow } from '@/components/tile-config';
...
import chromeDict from '@folio-lib/i18n/chrome';
...
export async function SlugTile({ slug, actor }: Props) {
  ...
  if (!tileRow) {
    ...
    const notFoundTitle: BilingualText = getTextServer(chromeDict, 'chrome.notFound.title', locale);
    const notFoundSubtitle: BilingualText = getTextServer(chromeDict, 'chrome.notFound.subtitle', locale);
    return (
      <>
        <BreadcrumbSetter crumbs={[ROOT_CRUMB, { label: `/${slug}` }]} />
        <PageLayout
          title={<T value={notFoundTitle} />}
          subtitle={<T value={notFoundSubtitle} />}
        >
          <NoPermissionView
            kind="not_found"
            actor={actor as any}
            attemptedPath={`/${slug}`}
            reason={<T value={notFoundSubtitle} />}
          />
        </PageLayout>
      </>
    );
  ...
  if (access.state === 'locked') {
    const locale = await getSecondaryLocale();
    const lockedBadge: BilingualText = getTextServer(chromeDict, 'chrome.tile.lockedBadge', locale);
    const accessRestricted: BilingualText = getTextServer(chromeDict, 'chrome.tile.accessRestricted', locale);
    return (
      <>
        <BreadcrumbSetter crumbs={[...tileCrumbs(tile), { label: <T value={lockedBadge} /> }]} />
        <PageLayout
          title={tile.display_name}
          subtitle={<T value={accessRestricted} />}
          category={{
