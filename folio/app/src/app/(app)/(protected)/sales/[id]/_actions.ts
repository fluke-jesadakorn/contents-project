...
import { withTransaction, query } from '@folio-lib/db';
import { recordEvent } from '@folio-lib/waybill/events';
import { appendWaybillEvent } from '@folio-lib/waybill/append';
...
import {
  ...
  finalizeSalesDraft,
} from '@folio-lib/finance/postSalesToGL';
...
export async function issueSalesInvoiceAction(formData: FormData): Promise<void> {
  ...
  const vat = await (await import('@folio-lib/finance/postSalesToGL')).loadDraftSalesJournal({ salesOrderId: so.id, step: 'sales_vat' });
  const accr = await (await import('@folio-lib/finance/postSalesToGL')).loadDraftSalesJournal({ salesOrderId: so.id, step: 'sales_accrual' });
...
async function advanceSalesOrderAction(args: { waybillId: string; target: string; event: import('@folio-lib/waybill/events').WaybillEventKind }): Promise<void> {
...
export async function attachArReceiptAction(formData: FormData): Promise<void> {
  ...
  const sett = await (await import('@folio-lib/finance/postSalesToGL')).loadDraftSalesJournal({ salesOrderId: so.id, step: 'sales_settlement' });
