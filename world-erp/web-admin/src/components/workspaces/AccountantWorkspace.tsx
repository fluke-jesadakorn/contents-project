          ...
          <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-800">
            <div>
              ...
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <span>📑</span> Receipt Audit Queue (Audit Queue)
              </h2>
            </div>
            <span className="px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 font-mono font-bold text-[10px] border border-indigo-500/30">
              {expenses.filter(e => e.status === 'ocr_extracted').length} pending
            </span>
          </div>

          <div className="space-y-2.5 max-h-[650px] overflow-y-auto pr-1">
            {expenses.length === 0 ? (
              <p className="text-center py-8 text-xs text-slate-500 font-mono">No reimbursement items in the system</p>
            ) : (
              expenses.map((exp) => {
                  ...
                  >
                    ...
                    <div className="text-xs mb-2">
                      ...
                      <span className="text-[10px] text-slate-400 block truncate font-mono">
                        Requester: {exp.submitter_name} ({exp.submitter_dept})
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-slate-900/80 font-mono">
                      ...
                      <span className="text-sm font-black text-white">
                        {parseFloat(exp.total_amount).toLocaleString('th-TH', { minimumFractionDigits: 2 })} THB
                      </span>
                    </div>
                    ...
                    {exp.is_corrupted && (
                      <div className="mt-2 text-[10px] bg-rose-500/10 border border-rose-500/20 text-rose-400 px-2 py-1 rounded font-bold flex items-center gap-1">
                        ⚠️ Sum of figures does not match accounting equation
                      </div>
                    )}
                  </div>
                );
        ...
        {selectedExpense && editForm ? (
            ...
            <div className="glass-panel-heavy p-6 rounded-3xl border-indigo-500/40 relative overflow-hidden shadow-2xl">
              <div className="flex justify-between items-center pb-4 border-b border-slate-800 mb-6">
                <div>
                  ...
                  <h3 className="text-base font-bold text-white mt-0.5 flex items-center gap-2">
                    <span>🔎</span> Audit & Assign COA (EXP-{selectedExpense.id})
                  </h3>
                </div>
              ...
              {isMathMismatch && (
                <div className="p-4 bg-rose-950/40 border-2 border-rose-500 rounded-2xl text-rose-300 text-xs mb-6 animate-pulse shadow-lg shadow-rose-950">
                  <div className="flex items-center gap-2 font-black text-sm text-rose-400 mb-1">
                    <span>🚨</span> Warning: Arithmetic Discrepancy
                  </div>
                  <p className="font-sans leading-relaxed">
                    Subtotal ({editForm.subtotal.toLocaleString()} THB) + VAT ({editForm.vatAmount.toLocaleString()} THB) equals <b>{(editForm.subtotal + editForm.vatAmount).toFixed(2)} THB</b> but the actual bill total is <b>{editForm.totalAmount.toLocaleString()} THB</b> (difference {Math.abs((editForm.subtotal + editForm.vatAmount) - editForm.totalAmount).toFixed(2)} THB). Please adjust the amounts below to balance.
                  </p>
                </div>
              )}

              {/* AI ANOMALY CHECK (acct:queue) */}
              <div className="mb-6 p-3 bg-indigo-950/20 border border-indigo-500/30 rounded-2xl">
                <div className="text-[10px] font-mono font-bold uppercase text-indigo-300 mb-2 tracking-wider">
                  🛡️ AI Anomaly Check
                </div>
                <AiActionButton
                  sectionKey="acct:queue"
                  task="chat"
                  systemPrompt="You are an expense auditor. Look at the receipt and identify anomalies: arithmetic mismatch, suspicious round numbers, off-hours dates (weekend/holiday), duplicate amounts, mismatch between vendor type and items, excessive tax (VAT ≠ 7%), and any other red flags. Be concise: list each anomaly in 1 line, then give a verdict: CLEAR, REVIEW, or BLOCK. Respond in the same language as the receipt text."
                  input={`Vendor: ${editForm.vendorName || 'unknown'}\nDate: ${editForm.transactionDate || 'unknown'}\nSubtotal: ${editForm.subtotal} THB\nVAT: ${editForm.vatAmount} THB\nTotal: ${editForm.totalAmount} THB\nPayment: ${editForm.paymentMethod}\nIsCorrupted: ${selectedExpense.is_corrupted}\nNotes: ${selectedExpense.correction_notes || 'none'}\nItems: ${(editForm.items || []).map((i: any) => `${i.description} (${i.amount} THB)`).join('; ')}`}
                  buttonLabel="Run AI anomaly check"
                  resultTitle="Anomaly Report"
                  tone="indigo"
                  glyph="🔍"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-amber-50 text-slate-900 p-5 rounded-2xl font-mono shadow-2xl border-l-4 border-indigo-600 text-xs relative flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-1.5 text-[11px] mb-4">
                      <div className="flex justify-between"><span className="text-slate-500">Date:</span> <span className="font-bold">{editForm.transactionDate || '--/--/----'}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500">Paid Via:</span> <span className="uppercase font-bold">{editForm.paymentMethod}</span></div>
                    </div>
                    ...
                    <div className="space-y-1.5 mb-4">
                      {editForm.items.map((item: any, i: number) => (
                        <div key={i} className="flex justify-between text-slate-900 text-[11px]">
                          ...
                          <span className="font-bold">{parseFloat(item.amount).toLocaleString('th-TH', { minimumFractionDigits: 2 })} THB</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-dashed border-slate-300 pt-3 space-y-1 text-slate-800">
                    <div className="flex justify-between text-[11px]"><span className="text-slate-500">SUBTOTAL:</span> <span>{editForm.subtotal.toLocaleString('th-TH', { minimumFractionDigits: 2 })} THB</span></div>
                    <div className="flex justify-between text-[11px]"><span className="text-slate-500">VAT (7%):</span> <span>{editForm.vatAmount.toLocaleString('th-TH', { minimumFractionDigits: 2 })} THB</span></div>
                    <div className="flex justify-between font-black text-sm border-t border-slate-900 pt-2 mt-1 text-indigo-950">
                      <span>TOTAL PAID:</span> <span>{editForm.totalAmount.toLocaleString('th-TH', { minimumFractionDigits: 2 })} THB</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3.5 bg-slate-950/70 p-4 rounded-2xl border border-slate-900 text-xs">
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-400 mb-1">Merchant</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-400 mb-1">Transaction Date</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-400 mb-1">Auditor Notes</label>
                    <textarea
                      rows={2}
                      value={editForm.correctionNotes}
                      onChange={(e) => setEditForm({ ...editForm, correctionNotes: e.target.value })}
                      placeholder="Provide reason for adjusting the equation or changing the account code..."
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="text-xs font-black text-white uppercase tracking-wider flex items-center gap-2 font-mono">
                      <span>🧠</span> Semantic COA Double-Entry Mapping
                      </h4>
                      <p className="text-[11px] text-slate-400 mt-0.5">AI BGE-M3 analyzes item names and recommends the correct account code per accounting standards</p>
                  </div>
                </div>
                  ...
                  {editForm.items.map((item: any, idx: number) => (
                    <div key={idx} className="bg-slate-950/80 p-4 rounded-2xl border border-slate-900 space-y-3">
                      <div className="flex justify-between items-center font-mono">
                        <span className="text-xs font-bold text-indigo-400">Item #{idx + 1}: {item.amount.toLocaleString()} THB</span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                        ...
                        <div className="md:col-span-5">
                          ...
                          >
                            <option value="">-- Suggest Account Code --</option>
                            {coa.map(c => (
                        ...
                        <div className="md:col-span-2">
                          ...
                          >
                            ...
                            <span>{loadingSuggestion[idx] ? 'Analyzing...' : 'AI Suggest'}</span>
                          </button>
                        </div>
                      </div>
                      ...
                      {suggestions[idx] && suggestions[idx].length > 0 && (
                        <div className="p-2.5 bg-indigo-950/30 rounded-xl border border-indigo-500/20 space-y-1.5 animate-fade-in">
                          <span className="text-[10px] text-indigo-300 font-bold block font-mono">
                            ✨ Semantically Similar Account Suggestions (Semantic Cosine Similarity):
                          </span>
              ...
              <div className="mt-8 pt-6 border-t border-slate-800 flex flex-col sm:flex-row gap-4">
                {selectedExpense.status === 'ocr_extracted' && (
                  ...
                  >
                    💾 Verify Equation & Send to Manager (Verify & Send to Manager)
                  </button>
                )}

                {selectedExpense.status === 'approved' && (
                  <div className="flex-1 px-4 py-3 rounded-2xl border border-emerald-500/30 bg-emerald-950/20 text-[11px] font-mono text-emerald-200/90">
                    ✅ Approved — awaiting Finance Disbursement desk to release payment.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="glass-panel p-12 rounded-3xl border border-slate-800 border-dashed text-center flex flex-col items-center justify-center h-[500px] text-slate-500">
            <span className="text-5xl mb-4">👩‍🔬</span>
              <h3 className="text-sm font-bold text-white">Auditor Workbench Idle (Auditor Idle)</h3>
              <p className="text-xs text-slate-400 mt-1 max-w-sm font-sans">
                Click a reimbursement item from the left queue to verify the arithmetic, assign COA, and post the entry to the GL
              </p>
          </div>
        )}
      </div>
