...
import tilesDict from '@erp-lib/i18n/tiles';
...
export const TilesClient: React.FC<Props> = ({ initialTiles, departments, canEdit }) => {
  ...
  const tTiles = useT(tilesDict);

  const tileNameBi = (t: TileRow): BilingualText => {
    const bi = tTiles(`tile.${t.id}.name`);
    if (bi.en === `tile.${t.id}.name`) {
      return { en: t.display_name, th: bi.th, de: bi.de };
    }
    return bi;
  };
  const [edits, setEdits] = useState<Record<string, EditState>>(() => {
    ...
    for (const t of initialTiles) {
      m[t.id] = { view_perm_id: t.view_perm_id ?? '' };
    }
    return m;
  });
  ...
  const dirtyIds = useMemo(
    () => initialTiles.filter((t) => isDirty(t, edits[t.id] || { view_perm_id: '' })).map((t) => t.id),
    [initialTiles, edits],
  );
  ...
  const resetRow = (id: string) => {
    const t = initialTiles.find((x) => x.id === id);
    if (!t) return;
    setEdits((prev) => ({
      ...prev,
      [id]: { view_perm_id: t.view_perm_id ?? '' },
    }));
    setError(null);
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <table className="w-full text-sm border-separate border-spacing-y-1">
        ...
        <tbody>
          {initialTiles.map((t) => {
            const dirty = isDirty(t, edits[t.id] || { view_perm_id: '' });
            const saving = savingId === t.id;
            return (
              <tr key={t.id} className="bg-slate-900/50 border border-slate-700">
                <td className="px-2 py-1.5 text-slate-100">{t.display_name}</td>
                <td className="px-2 py-1.5 text-slate-400 text-xs">{t.group_name}</td>
                <td className="px-2 py-1.5">
                  <input
                    type="text"
                    className="w-full px-2 py-1 rounded bg-slate-950 border border-slate-700 text-xs font-mono text-slate-100"
                    value={edits[t.id]?.view_perm_id ?? ''}
                    onChange={(e) => setViewPerm(t.id, e.target.value)}
                    disabled={!canEdit}
                  />
                </td>
      ...
      {departments.length > 0 ? (
        <details className="text-xs text-slate-400">
          <summary className="cursor-pointer">Known departments ({departments.length})</summary>
