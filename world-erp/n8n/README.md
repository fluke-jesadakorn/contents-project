...
- `world-erp/ocr/` — Fastify OCR service (port 3004)
- `world-erp/web-admin/src/app/api/upload/` — upload proxy
- `world-erp/web-admin/src/app/api/ai/invoke/` — AI router (called via `x-ai-key` header from the OCR service)
...
2. Forward image as base64 to `web-admin/api/ai/invoke` with `sectionKey: 'staff:ocr'` and `task: 'vision'`
