...
`web-admin/src/app/actions.ts:441` (expense) and `:809` (PR) throw when an actor's RBAC role does not have `update` on the stage module matching the expense's current `status`.
...
### 2. Chain picker — `web-admin/src/app/actions.ts`
...
`ceoForceDecision()` at `web-admin/src/app/actions.ts:493` writes to `ceo_overrides` + `approval_logs.stage='ceo_override'`. Gate is `requireActionFor(actorId, 'ceo_override', …)`. No change.
...
2. Check grants:
   ...
   ```
3. Static: `cd world-erp/web-admin && bun run lint && bunx tsc --noEmit`
